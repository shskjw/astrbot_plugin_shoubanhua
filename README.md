# 拉爆了，支持gemini官方api
大更新晚点写文档，一定要看
加了点新功能，比如在预设里加本地图片路径可以传入插件（只适配Windows），更好的lm列表，在命令后面加%，后面写的东西可以插入到预设内容的后面，还有增强词（比如pro）可以放到前面后面都行（纯符合我个人习惯），还可以艾特多个人获取他们的头像。
准备加点别的功能，但是不知道有啥加的，想试试下载图片失败返回下载链接，还有在预设里加图片公网链接可以把图片传入插件
功能基本上是ai写的，ai真是太好用了，希望作者看看能不能合并
